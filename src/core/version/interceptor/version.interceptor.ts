...
export class VersionInterceptor implements NestInterceptor<Promise<any>> {
    ...
    async intercept(context: ExecutionContext, next: CallHandler): Promise<Observable<Promise<any> | string>> {
        if (context.getType() === 'http') {
            ...
            const globalPrefix: boolean = this.configService.get<boolean>('app.globalPrefix');
            const versioning: boolean = this.configService.get<boolean>('app.versioning.on');
            const versioningPrefix: string = this.configService.get<string>('app.versioning.prefix');
            const request = ctx.getRequest<IRequestApp>();
            const originalUrl: string = request.url;

            if (versioning && originalUrl.startsWith(`${globalPrefix}/${versioningPrefix}`)) {
