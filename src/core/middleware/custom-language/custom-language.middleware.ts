...
import { HelperArrayService } from 'src/utils/helper/services/helper.array.service';
...
export class CustomLanguageMiddleware implements NestMiddleware {
    ...
    async use(req: IRequestApp, res: Response, next: NextFunction): Promise<void> {
        ...
        const enumLanguage: string[] = Object.values(await this.messageService.getLanguages());
        if (reqLanguages) {
            const languages: string[] = this.helperArrayService.unique(
                reqLanguages.split(',').filter((val) => this.helperArrayService.includes(enumLanguage, val)),
            );
