...
} from '../interfaces/helper-date-options.interface';
...
export class HelperDateService {
    ...
    calculateAge(dateOfBirth: Date, options?: IHelperDateOptions): number {
        return moment.tz(options && options.timezone ? options.timezone : this.timezone).diff(dateOfBirth, 'years');
    }

    diff(dateOne: Date, dateTwo: Date, options?: IHelperDateOptionsDiff): number {
        const mDateOne = moment.tz(dateOne, options && options.timezone ? options.timezone : this.timezone);
        const mDateTwo = moment.tz(dateTwo, options && options.timezone ? options.timezone : this.timezone);
    ...
    create(options?: IHelperDateOptionsCreate): Date {
        return moment
            .tz(options && options.date ? options.date : undefined, options && options.timezone ? options.timezone : this.timezone)
            .toDate();
    }
    ...
    format(date: Date, options?: IHelperDateOptionsFormat): string {
        return moment
            ...
            .format(options && options.format ? options.format : ENUM_HELPER_DATE_FORMAT.DATE);
    }

    forwardInMilliseconds(milliseconds: number, options?: IHelperDateOptionsForward): Date {
        return moment
            .tz(options && options.fromDate ? options.fromDate : undefined, options && options.timezone ? options.timezone : this.timezone)
            .add(milliseconds, 'ms')
            .toDate();
    }
    ...
    endOfMonth(month: number, options?: IHelperDateOptionsMonth): Date {
        const year =
            options && options.year ? options.year : moment.tz(options && options.timezone ? options.timezone : this.timezone).year();
        return moment
            .tz(options && options.timezone ? options.timezone : this.timezone)
            .year(year)
            .month(month - 1)
