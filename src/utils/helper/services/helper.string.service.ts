...
import { IHelperStringRandomOptions } from "../interfaces/helper-string-random-options.interface";
    ...
    random(length: number, options?: IHelperStringRandomOptions): string {
        const rString =
            options && options.safe
                ? faker.internet.password({ length, memorable: true, pattern: /[A-Z]/, prefix: options.prefix })
                : faker.internet.password({ length, memorable: true, pattern: /\w/, prefix: options.prefix });
