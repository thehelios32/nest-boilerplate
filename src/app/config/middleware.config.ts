...
export const middlewareConfig = registerAs(
    'middleware',
    (): Record<string, any> => ({
        cors: {
            allowMethod: [
        ...
        timestamp: {
            toleranceTimeInMs: process.env.MIDDLEWARE_TOLERANCE_TIMESTAMP ? ms(process.env.MIDDLEWARE_TOLERANCE_TIMESTAMP) : ms('5m'), // 5 mins
        },
        cache: {
            ttl: ms('30s'), // 30sec
        ...
        timeout: {
            in: process.env.MIDDLEWARE_TIMEOUT ? ms(process.env.MIDDLEWARE_TIMEOUT) : ms('30s'), // 30s based on ms module
        },
    }),
);
